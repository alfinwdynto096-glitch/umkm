...
<body class="bg-background font-body-md text-on-background antialiased">
    ...
    <nav class="bg-white/95 backdrop-blur-md fixed top-0 w-full z-50 border-b border-slate-200 shadow-sm">
        <div class="max-w-[1320px] mx-auto flex items-center justify-between px-6 py-4">
            <a href="{{ route('beranda') }}" class="text-xl font-bold text-emerald-900 tracking-tighter">UMKM
                Digital</a>
            ...
            <div class="hidden lg:flex items-center gap-6 font-inter text-sm font-medium">
                ...
                <a class="text-slate-600 hover:text-emerald-700 transition-colors"
                    href="{{ route('beranda') }}#kategori">Kategori</a>
            ...
            <div class="flex items-center gap-4 ml-6">
                ...
                <a href="{{ route('login') }}"
                    class="bg-primary text-on-primary px-6 py-2 rounded-xl font-button active:scale-95 transition-transform">Masuk</a>
            </div>
        </div>
    </nav>
    ...
    <section class="pt-20 bg-primary-container relative overflow-hidden">
        <div class="absolute inset-0 z-0">
            <div
                class="absolute inset-0 bg-gradient-to-br from-primary-container via-primary-container/90 to-tertiary-container/80">
            </div>
            <div
                class="absolute top-0 right-0 w-96 h-96 bg-primary-fixed/10 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl">
            </div>
            <div
                class="absolute bottom-0 left-0 w-64 h-64 bg-secondary-container/10 rounded-full translate-y-1/2 -translate-x-1/3 blur-3xl">
            </div>
        </div>
        <div class="relative z-10 max-w-[1320px] mx-auto px-6 py-14">
            ...
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
                <div>
                    ...
                    <p class="font-body-lg text-white/80 max-w-xl">Jelajahi ribuan produk lokal pilihan dari mitra UMKM
                        terpercaya di seluruh Indonesia.</p>
                    <div class="flex flex-wrap gap-3 mt-5">
                        <span
                            class="flex items-center gap-1.5 bg-white/10 text-white text-sm px-4 py-2 rounded-full backdrop-blur-sm">
                ...
                <div class="w-full lg:w-auto lg:min-w-[380px]">
                    <div class="bg-white rounded-xl shadow-xl p-2 flex items-center gap-2">
                        ...
                        <input id="searchHero" type="text" placeholder="Cari produk UMKM..."
                            class="flex-1 py-2 px-2 outline-none text-on-surface font-body-md bg-transparent placeholder-slate-400" />
                        <button
                            class="bg-primary text-white px-5 py-2.5 rounded-lg font-button text-sm active:scale-95 transition-transform">Cari</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white border-b border-slate-200 sticky top-[65px] z-40 shadow-sm">
        <div class="max-w-[1320px] mx-auto px-6 py-3 flex items-center gap-3 overflow-x-auto scrollbar-none">
            ...
            @foreach ($cats as $cat)
                <button
                    class="flex items-center gap-1.5 whitespace-nowrap px-4 py-2 rounded-full text-sm font-semibold transition-all {{ isset($cat['active']) ? 'bg-primary text-white shadow-sm' : 'bg-slate-50 text-slate-600 hover:bg-primary-fixed/40 hover:text-primary border border-slate-200' }} filter-cat-btn">
        ...
        <div class="flex flex-col lg:flex-row gap-gutter">
            ...
            <aside class="w-full lg:w-72 shrink-0">
                <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-lg sticky top-32">
                    ...
                    <div class="mb-md">
                        <p
                            class="font-label-bold text-label-bold text-on-surface-variant uppercase tracking-wider mb-sm">
                            Rentang Harga</p>
                        ...
                        <div class="space-y-2">
                            @foreach ([5, 4, 3] as $r)
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="rating" class="accent-primary"
                                        {{ $r == 5 ? 'checked' : '' }} />
                                    <div class="flex gap-0.5">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <span
                                                class="material-symbols-outlined text-sm {{ $i <= $r ? 'filled' : '' }} {{ $i <= $r ? 'text-secondary' : 'text-slate-300' }}">star</span>
                                        @endfor
                                    </div>
                    ...
                    <button
                        class="w-full py-3 bg-primary text-white rounded-xl font-button text-sm active:scale-[0.98] transition-all">
                        Terapkan Filter
                    </button>
                    <button
                        class="w-full py-3 mt-2 border border-slate-200 text-on-surface-variant rounded-xl font-button text-sm hover:bg-slate-50 transition-all">
                        Reset
                    </button>
                </div>
            </aside>
            ...
            <div class="flex-1 min-w-0">
                ...
                <!-- Sort bar -->
                <div
                    class="bg-white rounded-xl border border-slate-100 shadow-sm px-md py-3 flex flex-wrap items-center justify-between gap-3 mb-gutter">
                    <span class="text-body-sm text-on-surface-variant">Menampilkan <strong
                            class="text-on-surface">48</strong> dari <strong class="text-on-surface">2.500+</strong>
                        produk</span>
                    <div class="flex items-center gap-3">
                        <label class="text-body-sm text-on-surface-variant">Urutkan:</label>
                        <select
                            class="text-sm border border-slate-200 rounded-lg px-3 py-1.5 bg-slate-50 outline-none focus:ring-2 focus:ring-primary">
                        ...
                        <div class="flex rounded-lg border border-slate-200 overflow-hidden">
                            <button class="p-2 bg-primary text-white transition-colors" id="btnGrid"
                                title="Grid">
                                <span class="material-symbols-outlined text-sm">grid_view</span>
                            </button>
                            <button class="p-2 bg-white text-slate-400 hover:bg-slate-50 transition-colors"
                                id="btnList" title="List">
                ...
                @php
                    $products = [
                        ...
                        [
                            ...
                            'sold' => 87,
                            'img' =>
                                'https://lh3.googleusercontent.com/aida-public/AB6AXuDYQeOuPAE9CaNQ1achmlWvn6YD05-O1N08fP24HzdKH6Ly_gfS6074g8XkCM0oAvAC3syttyYoejGNFZ856Ve4Ds3xj0AZkzeJB6yncgen-WVTQLy7r9KyukzQemVd1q8STadPGMB5hpsJvC7f99nxZDqdgAwlptOWYJ3OihiNP9Ky-gl1Wv2tijywRRpXWaSX8pJIz10rN5GHHTbJaSywEmHuXLRpWPbSSHDi2j3-J9i5R606iNv6kydSD9VHbVqQQObzgGlvyR1W',
                        ],
                        [
                            'name' => 'Kopi Arabika Sangrai',
                            'cat' => 'Minuman Lokal',
                            'price' => '68.000',
                            'old' => '79.000',
                        ...
                        [
                            ...
                            'sold' => 130,
                            'img' =>
                                'https://lh3.googleusercontent.com/aida-public/AB6AXuC6QN1ZEIkdcJAWJuTqwQruR75a6x-j9CMYb9tyLdoIrYVi2ERMOWm1UWrQRi1qTxmS3XzyFcEww4o1eD6Wg05pOPwm7auZLmzWlnxux7Nzc6nBu-EpQqLzcfiwJX-ChCRepGURuL6my3WoeJXuimAjCg5-1YIk6gs2F0p8FtaRRFFMCUhAXshjm1DHGQnyWD5Q0MtMVlUO_2lSF8pBKNqFrbmh1pTW1aNCJBEyYSdjvajs5GLTfsAaaoFgI0sOguYSUAt-ycsDAuvH',
                        ],
                        [
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-gutter" id="productGrid">
                    @foreach ($products as $p)
                        <div
                            class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all group border border-slate-100 product-card">
                            <div class="relative h-56 overflow-hidden">
                                ...
                                <!-- badge -->
                                <span
                                    class="absolute top-3 left-3 bg-secondary-container text-on-secondary-container text-xs font-bold px-2.5 py-1 rounded-full">{{ $p['badge'] }}</span>
                                <!-- wishlist -->
                                <button
                                    class="wishlist-btn absolute top-3 right-3 bg-white/90 p-2 rounded-full text-primary hover:bg-white transition-colors active:scale-90">
                                    <span class="material-symbols-outlined text-[20px]">favorite</span>
                                </button>
                            </div>
                            <div class="p-md">
                                <span
                                    class="text-xs font-label-bold text-outline uppercase tracking-wider">{{ $p['cat'] }}</span>
                                <h4 class="font-label-bold text-base mt-1 text-on-surface line-clamp-1">
                                    {{ $p['name'] }}</h4>
                                <div class="flex items-center gap-1 my-2">
                                    <span
                                        class="material-symbols-outlined text-secondary text-[16px] filled">star</span>
                                ...
                                <div class="flex items-center justify-between mt-3">
                                    <div>
                                        ...
                                        @if ($p['old'])
                                            <span class="text-xs text-outline line-through ml-1">Rp
                                                {{ $p['old'] }}</span>
                                        @endif
                                    </div>
                                    <button
                ...
                <div class="mt-xl flex items-center justify-center gap-2">
                    <button
                        class="p-2 rounded-xl border border-slate-200 text-slate-400 hover:bg-slate-50 transition-colors disabled:opacity-40"
                        disabled>
                        <span class="material-symbols-outlined">chevron_left</span>
                    </button>
                    @foreach ([1, 2, 3, '…', 12] as $page)
                        <button
                            class="px-4 py-2 rounded-xl text-sm font-semibold transition-all {{ $page == 1 ? 'bg-primary text-white shadow-sm' : 'border border-slate-200 text-on-surface-variant hover:bg-slate-50' }}">{{ $page }}</button>
                    @endforeach
                    <button
                        class="p-2 rounded-xl border border-slate-200 text-on-surface hover:bg-slate-50 transition-colors">
        ...
        <div class="max-w-[1320px] mx-auto px-6">
            <div class="bg-emerald-900 rounded-[2rem] p-xl lg:p-xxl relative overflow-hidden">
                ...
                <div class="relative z-10 max-w-2xl">
                    ...
                    <p class="font-body-lg text-emerald-100/90 mb-8">Bergabunglah dengan ribuan mitra UMKM dan perluas
                        jangkauan pasar Anda ke seluruh Indonesia.</p>
                    <a href="{{ route('daftar') }}"
                        class="inline-block bg-secondary-container text-on-secondary-container px-8 py-4 rounded-xl font-button shadow-lg active:scale-95 transition-transform">
    ...
    <footer class="w-full border-t border-slate-200 bg-slate-50">
        <div class="max-w-[1320px] mx-auto py-12 px-6 flex flex-col md:flex-row justify-between items-start gap-8">
            <div class="max-w-xs">
                <a href="{{ route('beranda') }}" class="block text-md font-bold text-emerald-900 mb-3">UMKM
                    Digital</a>
                <p class="text-xs text-slate-500 leading-relaxed">Platform marketplace terpercaya yang menghubungkan
                    produk unggulan UMKM Indonesia dengan konsumen global.</p>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-10 w-full md:w-auto">
                <div class="flex flex-col gap-3">
                    ...
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors"
                        href="{{ route('beranda') }}">Etalase</a>
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors"
                        href="{{ route('beranda') }}#kategori">Kategori</a>
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors"
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors" href="#">Pusat
                        Bantuan</a>
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors"
                        href="#">Kontak</a>
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors" href="#">Lacak
                        Pesanan</a>
                </div>
                <div class="flex flex-col gap-3">
                    <span class="font-label-bold text-on-surface text-sm">Legal</span>
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors" href="#">Syarat
                        &amp; Ketentuan</a>
                    <a class="text-xs text-slate-500 hover:text-emerald-600 transition-colors"
        ...
        <div
            class="max-w-[1320px] mx-auto px-6 py-6 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
            <p class="text-xs text-slate-500">© 2026 UMKM Digital Indonesia. Memberdayakan Ekonomi Lokal.</p>
            <div class="flex gap-4">
                <span
                    class="material-symbols-outlined text-slate-400 cursor-pointer hover:text-primary transition-colors">social_leaderboard</span>
                <span
                    class="material-symbols-outlined text-slate-400 cursor-pointer hover:text-primary transition-colors">retweet</span>
                <span
                    class="material-symbols-outlined text-slate-400 cursor-pointer hover:text-primary transition-colors">video_youtube</span>
            </div>
        </div>
    </footer>
    ...
    <script>
        ...
        btnGrid && btnGrid.addEventListener('click', () => {
            ...
            btnGrid.classList.remove('bg-white', 'text-slate-400');
            btnList.classList.add('bg-white', 'text-slate-400');
        ...
        btnList && btnList.addEventListener('click', () => {
            ...
            btnList.classList.remove('bg-white', 'text-slate-400');
            btnGrid.classList.add('bg-white', 'text-slate-400');
